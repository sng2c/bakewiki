...
function llmHelpCommand(): void {
	emit({
		command: "llm",
		description: "LLM-friendly interface for bakewiki. 'get' outputs Markdown with YAML frontmetadata; all other commands output JSON on stdout.",
		options: {
		...
		subcommands: {
			...
			health: { args: [], description: "Health check", response: { ok: true } },
			file: {
				description: "Manage uploaded files",
				subcommands: {
					"file list": { args: ["[--slug <slug>]"], description: "List files (optionally filter by page slug)", response: { count: "number", files: [{ url: "path", filename: "slug/name", original: "name", ext: "jpg", path: "parent dir", slug: "full id", size: 12345 }] } },
					"file upload": { args: ["<file|->", "[name]", "[--slug <slug>]"], description: "Upload a file (use - for stdin)", response: { url: "path", filename: "slug/name", original: "name", ext: "jpg", path: "parent dir", slug: "full id", size: 12345 } },
					"file download": { args: ["<url|filename>", "[output|-]"], description: "Download a file" },
					"file delete": { args: ["<filename>"], description: "Delete a file", response: { deleted: "filename" } },
				},
			},
