...
// API 키 (bk_ 접두사): opaque 토큰, auth.json에서 직접 조회
// 세션: JWT 서명 검증 + allowlist(touchToken) 확인 → user 주입 (선택적, 없으면 통과)
export async function auth(c: Context<{ Variables: { store: Store; user: AuthUser | null } }>, next: Next) {
	...
	if (raw) {
		...
		if (raw.startsWith("bk_")) {
			// Opaque API 키: auth.json에서 직접 조회
			const result = touchToken(store, raw);
			if (result && result.type === "api") {
				c.set("user", { id: result.userId });
			}
		} else {
			// 세션 JWT: 서명 검증 + allowlist 확인
