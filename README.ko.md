...
실행 중인 서버를 CLI로 관리 — 문서 조회/생성/이름변경/부분업데이트/삭제, 검색, 업로드. → **[원격 CLI 전체 참조 →](docs/remote.md)** (영어)
...
bakewiki를 LLM 에이전트에 연결하고 싶다면, 바로 쓸 수 있는 **[pi 스킬 →](SKILL.md)**(세팅 + `bakewiki llm` 사용법, 스킬 프런트매터 포함)을 활용하세요. pi가 자동으로 발견하고 트리거합니다.
...
제목 해석: 제목은 항상 slug의 마지막 세그먼트입니다. 페이지 렌더 시 `# <제목>`이 H1으로 본문 앞에 추가되므로, 본문에는 `#` 헤딩을 넣지 않습니다.
